...
WorkDir = "/home/builder/BUILDDIR"
SCHEDULE_HOST = "solus-www"
LOG_HOST = "solus-www"
FERRYD_HOST = "solus-ring0"
BASE_REPO = "ssh://dev.getsol.us/source"
...
class Builder():
    ...
    def report_status(self, id, status):
        try:
            os.system("ssh -p 798 -4 build@%s status %s %s" % (SCHEDULE_HOST, id, status))
        except Exception, e:
            print e
            sys.exit(-1)
    ...
    def run(self):
        while True:
            ...
            output = None
            try:
                output = check_output("ssh -p 798 -4 build@%s get" % SCHEDULE_HOST)
            except Exception, e:
                time.sleep(10)
                continue
            if "\t" not in output:
            ...
            # submodules
            sbcmd = "git submodule update --init --recursive"
            try:
                os.chdir(clone_dir)
                check_output(sbcmd)
                os.chdir(clone_base)
            ...
            os.chdir(builddir)
            cmd = "sudo solbuild build -d --transit-manifest %s -n -p unstable-x86_64 -t -m 128G %s > \"%s.log\" 2>&1" % (targetRepo, tgt, tag)
            ...
            try:
                check_output("scp -4 %s %s ferryd@%s:root/incoming" % (" ".join(pkgs), trams[0], FERRYD_HOST))
            except Exception, e:
                print e
                self.report_status(id, "FAILED")
                time.sleep(10)
...
def sync_logs(tag, skip=False):
    try:
        ...
        check_output("scp -4 \"%s.log.gz\" logs@%s:logs/." % (tag, LOG_HOST))
    except Exception, e:
        print e
        if not skip:
            self.report_status(id, "FAILED")
