...
def configure_boot():
    ...
    cmd = "dracut --strip --hardlink --force "
...
def create_efi(title, name, label):
    ...
    cmd = "mkfs -t vfat -n SOLUSESP \"%s\"" % get_efi_path()
    try:
        r = check_call(cmd)
    except Exception, ex:
    ...
    try:
        # Install sd-boot as grubx64.efi for shim to load.
        sdboot_target = os.path.join(get_efi_root(), "EFI/Boot/grubx64.efi")
        shimefi_target = os.path.join(get_efi_root(), "EFI/Boot/BOOTx64.EFI")
        mokmanager_target = os.path.join(get_efi_root(), "EFI/Boot/mmx64.efi")
        mokcert_target = os.path.join(get_efi_root(), "solus-enroll-me.cer")
        d = os.path.dirname(sdboot_target)
        if not os.path.exists(d):
            os.makedirs(d)
        shutil.copy("/usr/lib/systemd/boot/efi/systemd-bootx64.efi", sdboot_target)
        shutil.copy("../infrastructure-tooling/image/shim-signed/shimx64.efi", shimefi_target)
        shutil.copy("../infrastructure-tooling/image/shim-signed/mmx64.efi", mokmanager_target)
        shutil.copy("../infrastructure-tooling/image/secureboot-keys/MOK.cer", mokcert_target)
    except Exception, ex:
        print("Failed to install systemd-boot & shim: %s" % ex)
        clean_exit(1)
    ...
    # Sign for secureboot
    # sbsign --key secureboot-keys/MOK.key --cert secureboot-keys/MOK.crt --output get_efi_root()/kernel get_efi_root()/kernel
    # sbsign --key secureboot-keys/MOK.key --cert secureboot-keys/MOK.crt --output get_efi_root()/grubx64.efi get_efi_root()/grubx64.efi
    try:
        kernel_target = os.path.join(get_efi_root(), "kernel")
        bootman_target = os.path.join(get_efi_root(), "EFI/Boot/grubx64.efi")
        subprocess.check_call(["sbsign", "--key", "../infrastructure-tooling/image/secureboot-keys/MOK.key", "--cert", "../infrastructure-tooling/image/secureboot-keys/MOK.crt", "--output", kernel_target, kernel_target])
        subprocess.check_call(["sbsign", "--key", "../infrastructure-tooling/image/secureboot-keys/MOK.key", "--cert", "../infrastructure-tooling/image/secureboot-keys/MOK.crt", "--output", bootman_target, bootman_target])
    except Exception, ex:
        print("Failed to sign boot files for secureboot, reason: %s" % ex)
        clean_exit(1)
...
def main():
    ...
    files["/usr/bin/sbsign"] = "sbsigntools"
